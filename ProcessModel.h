// Copyright 2024-2026 Board of Regents of the University of Wisconsin System
...
template <std::size_t Dim> class SlewModel {
  ...
  public:
    explicit SlewModel(ValueType origin, ValueType target,
        ...
        : origin_(origin), target_(target), displacement_([this] {
              ValueType d;
              std::transform(target_.cbegin(), target_.cend(),
                             origin_.cbegin(), d.begin(),
          ...
          rRate_([this, absRRate] {
              ...
              std::transform(displacement_.cbegin(), displacement_.cend(),
                             rr.begin(), [absRRate](auto d) {
                                 return d < 0.0 ? -absRRate : absRRate;
                             });
              return rr;
          }()),
          duration_([this] {
    ...
    ValueType Origin() const { return origin_; }

    ValueType Target() const { return target_; }

    auto StartTime() const { return startTime_; }

    auto FinishTime() const { return finishTime_; }

    template <typename TimePoint> ValueType ValueAtTime(TimePoint tp) const {
...
template <std::size_t Dim> class AsyncProcessModel {
    using ValueType = std::array<double, Dim>;
    ...
    template <typename TimePoint> bool CancelableWaitUntil(TimePoint timeout) {
  ...
  public:
    ...
    double ReciprocalSlewRateSeconds() const { return rRate_.count(); }
    ...
    double UpdateIntervalSeconds() const { return updateInterval_.count(); }
